...
#endif
/*hs14 code for SR-AL6528A-01-527 by liudi at 20220915 start*/
#define TP_INFO_SIZE         50
#define CTS_INFO_PROC_FILE "tp_info"
struct chipone_ts_data *tp_cts_data = NULL;
/*hs14 code for SR-AL6528A-01-527 by liudi at 20220915 end*/
...
#define CTSINFO_NAME "tp_wake_switch"
#define CTS_RANGE_TEST_OUT_PATH  "/data/tpdata/"
...
static int cts_resume(struct chipone_ts_data *cts_data)
{
    ...
    if (tp_cts_data != NULL) {
        cts_info("IC_NAME:NL9922C_JZ_TM TOUCH_VER:%02x\n", tp_cts_data->cts_dev.fwdata.version & 0xFF);
    }
...
static int esd_recovery_notifier_callback(struct notifier_block *nb, unsigned long action, void *data)
{
    const struct cts_platform_data *pdata = container_of(nb, struct cts_platform_data, esd_recovery_notifier);
    struct chipone_ts_data *cts_data = container_of(pdata->cts_dev, struct chipone_ts_data, cts_dev);

    cts_info("esd recovery notifier callback");

    if (action == FB_BLANK_UNBLANK) {
        queue_work(cts_data->workqueue,&cts_data->ts_resume_work);
    } else if (action == FB_BLANK_POWERDOWN) {
        cts_suspend(cts_data);
    }
    return 0;
}
...
static int cts_init_pm_fb_notifier(struct chipone_ts_data *cts_data)
{
    ...
    cts_data->pdata->esd_recovery_notifier.notifier_call = esd_recovery_notifier_callback;
    retval = register_esd_tp_recovery_notifier(&cts_data->pdata->esd_recovery_notifier);
    if (retval) {
        cts_err("register_esd_tp_recovery_notifier fail\n");
    }
#ifdef CFG_CTS_DRM_NOTIFIER
    {
        int ret = -ENODEV;
...
#else
    unregister_esd_tp_recovery_notifier(&cts_data->pdata->esd_recovery_notifier);
...
static struct platform_device ctsinfo_device = {
    .name = CTSINFO_NAME,
    .id = -1,
};

static ssize_t cts_ito_test_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    int ret = 0;

    cts_info("%s():start",__func__);

    if ((dev_mkdir(CTS_RANGE_TEST_OUT_PATH, 0777)) != 0) {
        cts_info("Failed to create directory for mp_test\n");
    }
...
static ssize_t cts_ito_test_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    int ret = 0;
    cts_info("%s():start",__func__);
    return ret;
}
static DEVICE_ATTR(factory_check, 0644, cts_ito_test_show, cts_ito_test_store);
static struct attribute *cts_ito_test_atts[] = {
        &dev_attr_factory_check.attr,
        NULL,
};
static const struct attribute_group cts_ito_test_attribute_group = {
    .attrs = cts_ito_test_atts,
};

static int cts_test_node_init(struct platform_device *ctsinfo_device)
{
    int err = 0;
    err = sysfs_create_group(&ctsinfo_device->dev.kobj,&cts_ito_test_attribute_group);
    if (0 != err) {
        cts_err("ERROR: chipone ITO node create failed.");
        sysfs_remove_group(&ctsinfo_device->dev.kobj,&cts_ito_test_attribute_group);
        return 0;
    } else {
        cts_err("chipone ITO node create_succeeded.");
    }
    return err;
}
/*hs14 code for SR-AL6528A-01-526 by hehaoran5 at 20220919 end*/
...
/*hs14 code for SR-AL6528A-01-527 by liudi at 20220915 start*/
static struct proc_dir_entry *cts_info_proc_entry;

static ssize_t cts_proc_getinfo_read(struct file *flip,char __user *buff,size_t size,loff_t *pPos)
{
    char buf[TP_INFO_SIZE] = {0};
    int rc = 0;
    if (tp_cts_data == NULL) {
        return -EFAULT;
    }

    snprintf(buf, sizeof(buf), "IC_NAME:NL9922C_JZ_TM TOUCH_VER:%02x\n", tp_cts_data->cts_dev.fwdata.version & 0xFF);
    rc = simple_read_from_buffer(buff,size,pPos,buf,strlen(buf));
    return rc;
}

static const struct file_operations cts_info_proc_fops = {
    .owner = THIS_MODULE,
    .read = cts_proc_getinfo_read,
};

static int32_t cts_extra_proc_init(void)
{
    cts_info_proc_entry = proc_create(CTS_INFO_PROC_FILE,0666,NULL,&cts_info_proc_fops);
    if (NULL == cts_info_proc_entry) {
        cts_err("%s():Couldn't create proc entry!\n",__func__);
        return -EFAULT;
    } else {
        cts_info("%s():Create proc entry success\n",__func__);
    }
    return 0;
}
/*hs14 code for SR-AL6528A-01-527 by liudi at 20220915 end*/
...
#endif
{
    ...
    /*hs14 code for SR-AL6528A-01-527 by liudi at 20220915 start*/
    if (cts_data != NULL) {
        tp_cts_data = cts_data;
    }
    cts_extra_proc_init();
    /*hs14 code for SR-AL6528A-01-527 by liudi at 20220915 end*/

    /*hs14 code for SR-AL6528A-01-526 by hehaoran5 at 20220919 start*/
    cts_info("%s(): start init ito test node",__func__);
    platform_device_register(&ctsinfo_device);
    cts_test_node_init(&ctsinfo_device);
    cts_info("%s(): ito test node init end",__func__);
    /*hs14 code for SR-AL6528A-01-526 by hehaoran5 at 20220919 end*/

/*hs14 code for SR-AL6528A-01-494 by hehaoran5 at 20220919 start*/
    ret = sec_cmd_init(&tp_cts_data->sec, cts_commands, cts_get_array_size(), SEC_CLASS_DEVT_TSP);
    if (ret < 0) {
        cts_err("%s: Failed to sec_cmd_init\n", __func__);
    } else {
        cts_info("%s: sec_cmd_init success!\n", __func__);
    }
...
/*hs14 code for SR-AL6528A-01-494 by hehaoran5 at 20220919 end*/
    /*hs14 code for SR-AL6528A-01-463 by zhangxiongyi at 20220926 start*/
    cts_data->pdata->tp_is_enabled = 1;
    ret = sysfs_create_link(&cts_data ->sec.fac_dev->kobj,&cts_data->pdata->ts_input_dev->dev.kobj, "input");
    if (ret < 0) {
        cts_err("create enable node fail %d", ret);
    }
    /*hs14 code for SR-AL6528A-01-463 by zhangxiongyi at 20220926 end*/
    return 0;
...
static int cts_detect_panel(void)
{
    ...
    if (strstr(saved_command_line, "lcd_nl9922c_jz_tm_mipi_fhd_video")) {
