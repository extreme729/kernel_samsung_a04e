...
int esd_recovery_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
    struct himax_ts_data *ts = container_of(self, struct himax_ts_data, esd_recovery_notifier);
...
static int himax_detect_panel(void)
{
    ...
    if (strstr(saved_command_line, "lcd_hx83112f_txd_boe_mipi_fhd_video")) {
        I("%s:panel = lcd_hx83112f_txd_boe_mipi_fhd_video\n",__func__);
        return 0;
    }
    return -EFAULT;
}
